...
### Κυκλοφορία 0.4.19 — το warning που αχρήστευε ολόκληρη απάντηση

- [x] **«Δοκιμή: Unexpected token '<'» — βρέθηκε η αιτία, και δεν ήταν η δοκιμή.**
      Ένα warning της PHP τυπωνόταν *πριν* από τη JSON και την αχρήστευε
      ολόκληρη· ο browser έλεγε μόνο «δεν είναι JSON». Συνέβαινε **μόνο** στην
      ...
      Αποδείχθηκε βάζοντας το ίδιο warning δίπλα-δίπλα στους δύο τρόπους
      εκκίνησης (`test_release_0_4_19.py`).
      - Τα σφάλματα PHP πάνε σε **αρχείο** (`php-errors.log`), ποτέ στο σώμα.
      - Το μοιραίο σφάλμα βγαίνει ως **JSON με το πραγματικό μήνυμα**
        (`register_shutdown_function`), αντί για άδεια απάντηση.
      - Το `apost` έκανε σκέτο `r.json()`. Πλέον κάθε μη-JSON απάντηση γίνεται
        αναγνώσιμη («HTTP 500 — …») μέσω του `apiParse`/`apiText`.
      - Και ο ίδιος ο έλεγχος δεν ακουμπά πια τον temp του **συστήματος**: το
        `tempnam` προειδοποιεί δυνατά όταν το TMP δεν είναι εγγράψιμο.
- [x] **Η αναζήτηση πιάνει όλες τις στήλες.** Στους «Πελάτες» έψαχνε ΑΦΜ και
      επωνυμία, ενώ ο πίνακας δείχνει δέκα στήλες· τώρα και κατάσταση, πλήθη,
      έσοδα/έξοδα, τελευταία λήψη — τα ποσά και όπως εμφανίζονται και
      ακατέργαστα, ώστε «1234» να βρίσκει το «1.234,56». Ίδια δουλειά στη
      «Λήψη» (πλήθος + ημερομηνία, και στις δύο μορφές).
- [x] **Στα Παραστατικά η στήλη «Πελάτης» με επωνυμία.** Ο πίνακας αναζήτησης
      της ΑΑΔΕ έχει **11 στήλες και καμία με όνομα** — μόνο ΑΦΜ αγοραστή. Η
      επωνυμία βγαίνει από το δικό μας πελατολόγιο (το ΑΦΜ μένει στο tooltip),
      από την κρυφή μνήμη και όχι με πλήρη συγχρονισμό: μια στήλη δεν αξίζει
      δευτερόλεπτα λευκού πίνακα. Ξαναγράφεται μόνη της όταν έρθει φρέσκο
      πελατολόγιο.
- [x] **Ο ιδιώτης μπορεί να πάρει ΑΦΜ, προαιρετικά.** Κενό επιτρέπεται, μισό
      ΑΦΜ όχι — φαίνεται έγκυρο και δεν είναι. Ο server το δεχόταν ήδη
      (`CustomerVat`)· έλειπε μόνο από τη φόρμα.
- [x] **Χρονοπρογραμματισμός: «Πελάτες», ΑΦΜ — επωνυμία, ορατά κουτάκια.**
      Το «031174383 · 031174383» ήταν σφάλμα: ο κώδικας ζητούσε στήλη `name`
      ενώ **η στήλη λέγεται `label`**. Ο έλεγχος ήταν πάντα ψευδής, κάθε
      πελάτης έπεφτε στο fallback «βάλε το ΑΦΜ» — και δεν έσκαγε τίποτα, γι'
      αυτό επέζησε.
- [x] **Τα κουτάκια επιλογής φαίνονται.** Οι λίστες δεν είχαν **κανέναν**
      κανόνα και έμεναν στο προεπιλεγμένο του Qt· και όπου υπήρχε κανόνας, το
      άδειο κουτάκι είχε περίγραμμα στο χρώμα των γραμμών (`#2b3b54` πάνω σε
      `#131f33`). Τώρα 2px στο χρώμα του σβησμένου κειμένου, παντού.
- [x] 19 νέα tests· σουίτα **740 passed / 13 skipped**.

### Κυκλοφορία 0.4.20 — ακύρωση με τους φόρους της, καρτέλα για ιδιώτη, εισαγωγή πληρωμών

- [x] **Το πιστωτικό πίστωνε 300 € παραπάνω απ' όσα τιμολογήθηκαν.** Αντέγραφε
      τις γραμμές του πρωτοτύπου αλλά **όχι τους φόρους του**. Στο ΜΑΡΚ
      400014973506434 (10.000 καθαρή, 2.400 ΦΠΑ, 300 παρακράτηση, **πληρωτέο
      12.100**) το πιστωτικό έβγαινε **12.400**. Δύο αιτίες:
      - Οι φόροι δεν αντιγράφονταν καθόλου (`$taxes` = `[]`). Τώρα αντιγράφονται
        όλοι — παρακρατούμενοι, τέλη, χαρτόσημο, λοιποί, κρατήσεις — και η
        **κατεύθυνση** κάθε φόρου έρχεται από την ΑΑΔΕ (`taxDecreaseTotalPaid`),
        δεν μαντεύεται: μια κράτηση μπορεί να μη μειώνει το πληρωτέο.
      - Το `ccr_grossValue` ήταν «καθαρή + ΦΠΑ». Δεν είναι: είναι το ΠΛΗΡΩΤΕΟ,
        και σε πιστωτικό είναι το νούμερο που η ΑΑΔΕ συγκρίνει με το
        συσχετιζόμενο. Επαληθεύτηκε ζωντανά, με προεπισκόπηση: **12.400 →
        12.100**, και μερική πίστωση 4.000 → παρακράτηση 120, σύνολο 4.840.
      - Τρίτο, στην ίδια διαδρομή: η **προεπισκόπηση αποθηκευμένου προχείρου**
        πετούσε κι αυτή τους φόρους (`tempTaxesToForm`).
- [x] **Καρτέλα για πελάτη χωρίς ΑΦΜ.** Η λίστα παραστατικών της ΑΑΔΕ έχει
      **κενό ΑΦΜ αγοραστή σε ΚΑΘΕ απόδειξη λιανικής** — τίποτα δεν ξεχωρίζει
      τον έναν ιδιώτη από τον άλλον. Ο κωδικός πελάτη όμως υπάρχει μέσα στο
      παραστατικό: κλειδί `#<κωδικός>`, μία κλήση ανά απόδειξη χωρίς ΑΦΜ, με
      μνήμη (0,7s → 0,2s στο δεύτερο άνοιγμα). Οι πληρωμές και το ZIP
      ακολουθούν το ίδιο κλειδί.
- [x] **Η αναζήτηση στους Πελάτες έβρισκε την πόλη — ποτέ.** Τα δεδομένα της
      ΑΑΔΕ είναι ΚΕΦΑΛΑΙΑ ΚΑΙ ΑΤΟΝΑ («ΒΑΡΗ»), ο χρήστης γράφει «Βάρη». Νέο
      `grFold` (χωρίς τόνους, χωρίς πεζά/κεφαλαία) σε όλες τις αναζητήσεις, και
      φιλτράρισμα καθώς γράφεις.
- [x] **Στα Παραστατικά: ημερομηνία, καθαρή, ΦΠΑ, σύνολο.** Τα ποσά και όπως
      γράφονται και σκέτα («12100» βρίσκει το «12.100,00»), η ημερομηνία και ως
      «24/08/2026» και ως «2026-08».
- [x] **«Η αναζήτηση πελάτη δεν φέρνει τους πελάτες».** Ένας αποτυχημένος
      συγχρονισμός **έσβηνε** την κρυφή μνήμη: το `cachedThenSync` έγραφε
      `rows||[]` πάνω σε καλά δεδομένα, ακόμη κι όταν ο server απαντούσε
      `success:false` (409). Αποδείχθηκε στον browser: παλιά λογική → **0**
      πελάτες, νέα → **28**.
- [x] **Το dropdown άργησε επειδή περίμενε την ΑΑΔΕ.** Νέο `ensureCustomers()`:
      κρυφή μνήμη πρώτα, συγχρονισμός από πίσω. Άνοιγμα φόρμας **9 ms**,
      επιλογέας **20 ms** (ήταν ολόκληρος γύρος στην ΑΑΔΕ, κάθε φορά).
- [x] **Ποια τράπεζα εντοπίστηκε — και γράφεται.** Η αναγνώριση γινόταν μόνο
      από το ΟΝΟΜΑ του αρχείου. Τώρα από τον κωδικό του IBAN μέσα στο αρχείο,
      από το κείμενό του, και —με προτεραιότητα— από τους **δικούς σου**
      λογαριασμούς (Ρυθμίσεις → 🏦). Η επιλογή τράπεζας δείχνει πλέον ΟΛΗ τη
      λίστα (20 τράπεζες) και επιλέγεται μόνη της.
- [x] **Η πληρωμή θυμάται πού μπήκαν τα χρήματα.** Δύο στήλες (`bank`,
      `bank_account`), σε αυτόματη και χειροκίνητη καταχώρηση· ο επιλογέας
      λογαριασμού εμφανίζεται μόνο όταν υπάρχουν καταχωρημένοι. Ταξιδεύουν και
      στον συγχρονισμό γραφείου↔server (χωρίς αλλαγή στο αποτύπωμα, ώστε να μη
      διπλασιαστεί καμία παλιά πληρωμή).
- [x] **Οι ημερομηνίες συμφωνούν.** Η καρτέλα ανακάτευε «24/08/2026»
      (τιμολόγιο) με «2026-08-24» (πληρωμή) στην ΙΔΙΑ στήλη. Και το
      `payment_date_iso` δεχόταν μόνο διψήφια ημέρα/μήνα: το «13/8/2026»
      αποθηκευόταν **αυτούσιο**, δηλαδή αόρατο σε κάθε φίλτρο διαστήματος — η
      πληρωμή υπήρχε στη βάση και έλειπε από το υπόλοιπο. Και σφάλμα που βρήκε
      το test: το `(\d{2}|\d{4})` δοκίμαζε πρώτα τα δύο ψηφία, οπότε το
      «13/08/2026» έδινε έτος **2020**.
- [x] 24 νέα tests· σουίτα **764 passed / 13 skipped**.

### Κυκλοφορία 0.4.21 — τα κλειδιά που δεν άνοιγαν, και το dropdown που δεν γέμιζε

- [x] **Το αντίγραφο ασφαλείας δεν κουβαλούσε το κλειδί του.** Τα credentials
      κρυπτογραφούνται με κλειδί που ζει ΕΞΩ από τη βάση (`.enckey`). Ένα
      αντίγραφο μόνο της βάσης είναι μισό: επαναφορά σε φάκελο που στήθηκε από
      την αρχή έφερνε τους πελάτες **με κλειδιά που δεν ανοίγουν**.
      Επαληθεύτηκε στα δεδομένα του χρήστη: 24/08 10:42 νέο `.enckey`, 10:43
      επαναφορά, **61 από 62** πελάτες κλειδωμένοι. Και οι επαναφορές που
      δοκίμασε στις 27/08 δεν άλλαζαν τίποτα — ξανάφερναν την ίδια βάση στο ίδιο
      λάθος κλειδί.
      - Κάθε αντίγραφο παίρνει πλέον μαζί το `.enckey` του (ίδιο όνομα).
      - Η επαναφορά το ξαναβάζει **μόνο** όταν το τρέχον δεν ανοίγει τα
        επαναφερμένα credentials ενώ του αντιγράφου τα ανοίγει: επαναφορά σε
        φάκελο που δουλεύει δεν μπορεί να τον χαλάσει.
      - Το κλειδί που αντικαθίσταται **φυλάγεται** (`.enckey-<ώρα>-pre-restore`)·
        ποτέ δεν σβήνουμε κλειδί — μπορεί να ανοίγει δεδομένα που δεν βλέπουμε.
      - `prune`: το κλειδί φεύγει μαζί με το αντίγραφό του (ορφανό κλειδί = ρίσκο).
- [x] **Η κατάσταση έλεγε ψέματα.** Η στήλη έβγαινε από το αν το *κρυπτογράφημα*
      είναι μη κενό, όχι από το αν διαβάζεται: 61 κλειδωμένοι πελάτες έδειχναν
      «Διαθέσιμος», η καρτέλα άνοιγε με άδεια πεδία, και η λήψη αποτύγχανε χωρίς
      να πει γιατί. Νέο `repo.unreadable_clients()`, ετικέτα «Κλειδί κλειδωμένο»
      και ειδοποίηση που λέει τι συνέβη και τι να κάνει ο χρήστης.
- [x] **Ο οδηγός έκδοσης ρωτούσε δύο φορές.** Διάλεγες «Ιδιώτης» και το «+ Νέος
      πελάτης» άνοιγε στην καρτέλα «Με ΑΦΜ». Ανοίγει πια στη σωστή, με εστίαση
      στο Ονοματεπώνυμο — και το πεδίο ΑΦΜ της φόρμας **καθαρίζει** (κρατώντας
      ό,τι είχε πληκτρολογηθεί, η απόδειξη έβγαινε σε άλλον).
- [x] **Μια αποτυχημένη ανανέωση ΕΣΒΗΝΕ τα είδη.** Το `buildProdMap(d.products||[])`
      έγραφε άδειο πίνακα πάνω στα είδη που μόλις είχαν φορτώσει από την κρυφή
      μνήμη, κάθε φορά που ο server απαντούσε `success:false` (π.χ. 409 «διάλεξε
      πρώτα εταιρεία»). Το dropdown της γραμμής έμενε με μόνο το «➕ Νέο είδος…»
      — αυτό ήταν το «δεν γίνεται hydrate». Νέο `freshRows()`: αποτυχία δεν
      αγγίζει τίποτα, επιτυχής άδεια απάντηση εφαρμόζεται. Ίδιος κανόνας σε
      είδη, σειρές (τρεις θέσεις) και πελάτες.
- [x] **Τα είδη μένουν στην κρυφή μνήμη.** Η ανανέωση περνά από το `sync`, όχι
      από το `list_products`: η ίδια κλήση γράφει και την κρυφή μνήμη. Πριν, τα
      είδη έμπαιναν εκεί μόνο αν είχες ανοίξει την οθόνη «Είδη».
- [x] **Καμία αναζήτηση δεν σκοντάφτει πια στον τόνο.** «Χρωματισμών» έδινε
      **0** αποτελέσματα, «χρωματισμων» έδινε 6: τα δεδομένα της ΑΑΔΕ είναι
      ΚΕΦΑΛΑΙΑ ΚΑΙ ΑΤΟΝΑ. Το `grFold` μπήκε και στους δέκα επιλογείς (είδη,
      πελάτες, μαζική έκδοση, δελτίο αποστολής, ακύρωση, αναθέσεις, φίλτρα).
- [x] 23 νέα tests· σουίτα **787 passed / 13 skipped**.

### Κυκλοφορία 0.4.22 — το dropdown ήταν αόρατο, όχι σπασμένο

- [x] **Η αιτία ήταν CSS, όχι JavaScript.** Το `.ac-panel` είναι
      `position:absolute` στο `top:100%` — δηλαδή ΕΞΩ από το κελί του — και ο
      κανόνας `table td{overflow:hidden}` (που υπάρχει για το ellipsis) το έκοβε
      ολόκληρο. Γι' αυτό κάθε έλεγχος σε JavaScript έδειχνε «ανοιχτό, 13
      επιλογές» ενώ στην οθόνη δεν υπήρχε τίποτα, και γι' αυτό ίσχυε σε ΚΑΘΕ
      πίνακα: έκδοση, μαζική έκδοση, δελτίο αποστολής, εισαγωγή πληρωμών.
      Νέα κλάση `td.ac-cell{overflow:visible}` στα πέντε κελιά που φιλοξενούν
      λίστα· ο κανόνας του ellipsis μένει για όλα τα υπόλοιπα.
      Επαληθεύτηκε με `elementFromPoint` μέσα στο dropdown: ΟΡΑΤΟ.
- [x] **Δεύτερο, ανεξάρτητο κόψιμο σε στενό παράθυρο.** Το
      `.panel table{overflow-x:auto}` κάνει τον πίνακα δοχείο κύλισης, που κόβει
      το ίδιο πράγμα. Εξαίρεση με `:has(td.ac-cell)`.
- [x] **«Αργεί να φορτώσει τις σειρές».** Ο τοπικός server της PHP εξυπηρετεί
      ΜΙΑ αίτηση τη φορά· το `prewarmAll` έκανε **επτά σειριακούς συγχρονισμούς
      με την ΑΑΔΕ** στην εκκίνηση και ΚΑΘΕ ενέργεια του χρήστη περίμενε πίσω
      τους. Πλέον ό,τι ρωτήθηκε τα τελευταία 10 λεπτά δεν ξαναρωτιέται: η
      ηλικία της κρυφής μνήμης έρχεται από τον server (`age_seconds`, UTC ρητά,
      ώστε ο browser να μη μαντεύει ζώνη ώρας). Επανεκκίνηση: οι λίστες είναι
      ήδη εκεί πριν προλάβει να μετρήσει ο μετρητής.
- [x] **Και μια παγίδα που έστησα μόνος μου, την οποία έπιασε το ζωντανό test:**
      η «φρεσκάδα» έκρυβε τις ΔΙΚΕΣ ΜΑΣ εγγραφές — έφτιαχνες είδος και δεν
      εμφανιζόταν, γιατί η μνήμη ήταν δύο λεπτών. Νέο `CACHE_DIRTY`: κάθε
      δημιουργία/διαγραφή (σειρά, είδος, πελάτης — έξι σημεία) σημαδεύει το
      είδος της, και ο έλεγχος φρεσκάδας ζει σε **μία** συνάρτηση ώστε να μην
      μπορεί να παρακαμφθεί.
- [x] **Ο έλεγχος ΑΑΔΕ κάθε 5 λεπτά** (ήταν 30): ένα παραστατικό που εκδόθηκε
      αλλού μπορούσε να μείνει μισή ώρα χωρίς ειδοποίηση — η καμπάνα το έλεγε
      ΜΕΤΑ από τον χρήστη.
- [x] **Νέο είδος/σειρά στην ΑΑΔΕ: επαληθεύτηκε ζωντανά.** Δημιουργήθηκε
      δοκιμαστικό είδος στον πραγματικό λογαριασμό, η λίστα της ΑΑΔΕ πήγε 12→13
      με το είδος μέσα, και διαγράφηκε (12 ξανά). Η δημιουργία ΕΙΝΑΙ άμεση· ό,τι
      έλειπε ήταν η ανανέωση της δικής μας μνήμης, που διορθώθηκε παραπάνω.
- [x] **Δοκιμή κωδικών:** έλεγε «ο μήνας … είναι χωρίς έξοδα», που διαβαζόταν σαν
      εύρημα. Το πλήθος εγγραφών δεν είναι αποτέλεσμα της δοκιμής των κλειδιών.
- [x] **Στα Είδη «Φίλτρο» → «Αναζήτηση κωδικού ή περιγραφής»**, και η αναζήτηση
      αγνοεί τόνους όπως όλες οι υπόλοιπες.
- [x] **Πιο έντονα περιγράμματα:** η αντίθεση του `--line` ήταν **1,3:1** — τα
      κουτιά δεν είχαν ορατό όριο. Τώρα **3,3:1** στο σκοτεινό και **2,6:1** στο
      φωτεινό, μετρημένα.
- [x] `showView` με άγνωστο όνομα δεν ρίχνει πια τη ροή (ο βοηθός βγάζει ονόματα
      οθονών από κείμενο του χρήστη).
- [x] **Έλεγχος όλων των οθονών** με τα διαπιστευτήρια του χρήστη: 14/14
      φορτώνουν χωρίς σφάλμα, μηδέν εγγραφές στο log σφαλμάτων της PHP.
- [x] 18 νέα tests· σουίτα **805 passed / 13 skipped**.
