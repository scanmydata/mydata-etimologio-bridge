...
<dialog id="uiAsk"><div class="modal-body" style="max-width:520px">
  ...
  <main>
    ...
    <section class="view" id="view-stats">
      <h2 class="title">Στατιστικά</h2><p class="sub">Σύνοψη τζίρου & παραστατικών από την ΑΑΔΕ. Κάθε ποσό γράφει αν είναι <b>καθαρή αξία</b> (χωρίς ΦΠΑ) ή <b>σύνολο με ΦΠΑ</b>· τα πιστωτικά αφαιρούνται.</p>
      <div class="panel">
        ...
        <table id="statTable"><thead><tr><th>Τύπος</th><th class="num">Πλήθος</th><th class="num">Καθαρή αξία (€)</th><th class="num">ΦΠΑ (€)</th><th class="num">Σύνολο με ΦΠΑ (€)</th><th>Μερίδιο (καθαρή)</th></tr></thead><tbody></tbody></table>
      </div>
    </section>
    ...
    <section class="view" id="view-customers">
      ...
      <div class="panel">
        <div class="row">
          <div class="field grow"><label>Αναζήτηση (ΑΦΜ / επωνυμία / κωδικός / πόλη)</label>
            <input id="custSearch" placeholder="Πληκτρολόγησε…" autocomplete="off" oninput="renderCustomers()"></div>
    ...
    <section class="view" id="view-documents">
      ...
      <div class="panel">
        <div class="row">
          ...
          <div class="field grow"><label>Αναζήτηση (ΑΦΜ / επωνυμία / ΜΑΡΚ / σειρά / ημερομηνία / ποσό)</label><input id="docSearch" placeholder="Πληκτρολόγησε…" autocomplete="off" oninput="renderDocs()"></div>
      ...
      <div class="panel" style="margin-top:16px">
        <div class="row">
          <div class="field"><label>Τράπεζα</label>
            <select id="biBank"><option value="">Αυτόματα</option></select></div>
        ...
        <!-- Τι αναγνωρίστηκε, με τα ίδια λόγια που θα γραφτεί στην πληρωμή. -->
        <div class="hint" id="biBankFound" style="display:none"></div>
...
<dialog id="payModal"><form method="dialog" class="modal-body" onsubmit="savePayment(event)">
  ...
  <div class="row"><div class="field"><label>Ημ/νία</label><input id="pmDate" type="text" class="dt" placeholder="ηη/μμ/εεεε" maxlength="10" oninput="dtMask(this)" required></div>
    <div class="field"><label>Τρόπος</label><select id="pmMethod"><option value="3">Μετρητά</option><option value="1">Τραπεζική μεταφορά</option><option value="4">Επιταγή</option></select></div>
    <!-- Εμφανίζεται μόνο όταν υπάρχουν καταχωρημένοι λογαριασμοί: με έναν
         λογαριασμό είναι θόρυβος, με τρεις είναι η μόνη πληροφορία που λείπει. -->
    <div class="field grow" id="pmBankWrap" style="display:none"><label>Λογαριασμός είσπραξης</label>
      <select id="pmBankAcc"><option value="">—</option></select></div></div>
...
// Ελληνικά χωρίς τόνους και χωρίς κεφαλαία, ώστε το «Βάρη» να βρίσκει το
// «ΒΑΡΗ». Τα δεδομένα της ΑΑΔΕ είναι ΚΕΦΑΛΑΙΑ ΚΑΙ ΑΤΟΝΑ, ενώ ο χρήστης γράφει
// πεζά και τονισμένα: χωρίς αυτό δεν συναντιόνταν ποτέ. Το τελικό «ς» γίνεται
// «σ» για τον ίδιο λόγο («Πάτρας» / «πατρασ»).
function grFold(s){return (s==null?'':String(s)).normalize('NFD')
  .replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\u03c2/g,'\u03c3');}
...
function showView(v){
  ...
  // `loadTaxCats` ΜΕΤΑ: ο τοπικός server εξυπηρετεί ένα αίτημα τη φορά, και οι
  // φόροι (σύνδεση στην ΑΑΔΕ) έμπαιναν μπροστά από τις σειρές του οδηγού.
  if(v==='issue'){if(!$('#iLines tbody').children.length)addLine();loadIssueTypes().finally(()=>setTimeout(loadTaxCats,300));renderTaxes();
    if(!issueHasContent()){WIZ_DONE=false;ISSUE_WHO='';}
    wizShow(!WIZ_DONE);}
  if(v==='drafts')loadDrafts();
  if(v==='bankimp'){ensureCustomers();loadBankSettings();biBankOptions();}
...
async function loadStats(){
  ...
  try{const d=await api({statistics:1,period:STAT_PERIOD});if(!d.success)throw new Error(d.error||'σφάλμα');
    $('#statCards').innerHTML=
      // Ρητά ΤΙ είναι κάθε ποσό: «τζίρος» σκέτο διαβαζόταν άλλοτε με και άλλοτε
      // χωρίς ΦΠΑ. Τα πιστωτικά αφαιρούνται.
      `<div class="card"><div class="k">Καθαρή αξία <span class="muted">(χωρίς ΦΠΑ)</span></div><div class="v money">${fmt(d.total_value)} €</div></div>`+
      (d.total_gross!==undefined?`<div class="card"><div class="k">ΦΠΑ</div><div class="v money">${fmt(d.total_vat)} €</div></div>`+
      `<div class="card"><div class="k">Σύνολο <span class="muted">(με ΦΠΑ)</span></div><div class="v money">${fmt(d.total_gross)} €</div></div>`:'')+
      `<div class="card"><div class="k">Πλήθος παραστατικών</div><div class="v">${d.total_count}</div></div>`;
    const max=Math.max(1,...d.breakdown.map(b=>Math.abs(b.value)));
    const hasGross=d.breakdown.some(b=>b.gross!==undefined);
    $('#statTable tbody').innerHTML=d.breakdown.map(b=>`<tr><td><span class="pill" title="${esc(invName(b.type))}">${esc(b.type)}</span> ${esc(invName(b.type))}</td><td class="num">${b.count}</td><td class="num">${fmt(b.value)}</td><td class="num">${hasGross?fmt(b.vat||0):'—'}</td><td class="num">${hasGross?fmt(b.gross||0):'—'}</td><td><div class="bar"><i style="width:${Math.round(Math.abs(b.value)/max*100)}%"></i></div></td></tr>`).join('')||'<tr><td colspan="6" class="muted">Δεν υπάρχουν δεδομένα.</td></tr>';
...
function barSvg(rows){
  ...
  return `<div class="hint" style="margin-bottom:6px">Ποσά: καθαρή αξία (χωρίς ΦΠΑ)</div><svg class="chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="Καθαρή αξία ανά τύπο παραστατικού">${bars}</svg>`+chartLegend(rows,total);
}
function pieSvg(rows){
  ...
  return `<div class="hint" style="margin-bottom:6px">Μερίδιο στην καθαρή αξία (χωρίς ΦΠΑ)</div><svg class="chart pie" viewBox="0 0 300 300" role="img" aria-label="Μερίδιο καθαρής αξίας ανά τύπο">${slices}</svg>`+chartLegend(rows,total);
}
...
async function cachedThenSync(kind,onRows){
  let shown=false,fresh=false;const dirty=CACHE_DIRTY.has(kind);
  try{const c=await api({cached:kind});if(c.rows&&c.rows.length){onRows(c.rows,true);shown=true;}
    fresh=cacheIsFresh(kind,c);}catch(e){}
  // Επιβεβαιωμένη πρόσφατα και όχι αλλαγμένη από εμάς: καμία κλήση στην ΑΑΔΕ.
  if(shown&&fresh&&!dirty)return;
  try{const s=await api({sync:kind,force:dirty?1:''});
    // ΠΟΤΕ πάνω σε καλά δεδομένα με άδεια. Ο συγχρονισμός απαντά `success:false`
    // (π.χ. 409 «διάλεξε πρώτα εταιρεία») ή γυρίζει άδειο σε στιγμιαία αστοχία
    // της ΑΑΔΕ — και μέχρι τώρα αυτό ΣΒΗΝΕ το πελατολόγιο που είχε ήδη φορτώσει
    // από την κρυφή μνήμη. Ο χρήστης το έβλεπε ως «η αναζήτηση δεν φέρνει
    // πελάτες», χωρίς κανένα μήνυμα λάθους πουθενά.
    if(s.success===false)throw new Error(s.error||'ο συγχρονισμός απέτυχε');
    const rows=s.rows||[];
    if(rows.length||!shown)onRows(rows,false);
    if(s.changed&&shown&&s.prev_count>0)toast('Ενημερώθηκε ('+kind+')','ok');
    if(kind==='customers'&&s.details_pending>0)custFillDetails();
  }catch(e){if(!shown)toast(kind+': '+e.message,'err');}
}

// Το πελατολόγιο ΤΩΡΑ, όχι σε πέντε δευτερόλεπτα.
//
// Κάθε επιλογέας πελάτη καλούσε `loadCustomers()`, που περιμένει και τον γύρο
// προς την ΑΑΔΕ. Το dropdown της μεμονωμένης πληρωμής άνοιγε άδειο και γέμιζε
// αργότερα· η αναζήτηση στην εισαγωγή αρχείου έψαχνε σε κενή λίστα. Εδώ
// διαβάζουμε ΜΟΝΟ την κρυφή μνήμη (ακαριαία) και αφήνουμε τον συγχρονισμό να
// τρέξει από πίσω, μία φορά.
let CUST_SYNC=null;
async function ensureCustomers(){
  if(ALL_CUSTOMERS.length)return ALL_CUSTOMERS;
  try{const c=await api({cached:'customers'});
    if(c.rows&&c.rows.length){ALL_CUSTOMERS=c.rows;if($('#custTable'))renderCustomers();}
  }catch(e){}
  if(!CUST_SYNC)CUST_SYNC=loadCustomers().catch(()=>{}).finally(()=>{CUST_SYNC=null;});
  if(!ALL_CUSTOMERS.length)await CUST_SYNC;    // άδεια μνήμη: αξίζει η αναμονή
...
function renderCustomers(){
  const term=grFold($('#custSearch').value||'').trim();
  let rows=ALL_CUSTOMERS.map(custFields);
  // Κάθε στήλη του πίνακα, και η διεύθυνση από πάνω: η «Πόλη» ήταν μπροστά στα
  // μάτια του χρήστη και δεν αναζητούνταν, γιατί ο τόνος που έγραφε δεν
  // υπάρχει ποτέ στα δεδομένα της ΑΑΔΕ.
  if(term)rows=rows.filter(c=>grFold([c.code,c.vat,c.name,c.city,c.address,c.type].join(' ')).includes(term));
  ...
  $('#custTable tbody').innerHTML=rows.slice(0,500).map(c=>
    `<tr class="clickable" onclick="openCard('${q1(c.vat)}','${q1(c.name)}','${q1(c.code)}')"><td>${esc(c.code)}</td><td>${esc(c.vat)}</td><td>${esc(c.name)}</td><td>${esc(c.city)}</td>
      ...
      <button class="ghost sm" onclick="event.stopPropagation();openCard('${q1(c.vat)}','${q1(c.name)}','${q1(c.code)}')">Καρτέλα →</button></td></tr>`).join('')||'<tr><td colspan="5" class="muted">Κανένα αποτέλεσμα.</td></tr>';
...
// Τ.Κ., email, τηλέφωνα, ΔΟΥ: η λίστα της ΑΑΔΕ δεν τα έχει — τα διαβάζει ο server
// από τη σελίδα κάθε πελάτη, λίγα κάθε φορά, και μένουν στη βάση.
let CUST_DETAILS_RUN=null;
function custFillDetails(){
  if(CUST_DETAILS_RUN)return CUST_DETAILS_RUN;
  CUST_DETAILS_RUN=(async()=>{
    for(let round=0;round<15;round++){
      let d;try{d=await api({sync:'customers',details:1});}catch(e){break;}
      if(!d||d.success===false)break;
      if(Array.isArray(d.rows)&&d.rows.length){ALL_CUSTOMERS=d.rows;if($('#custTable'))renderCustomers();}
      if(!(d.details_pending>0))break;
    }
  })().finally(()=>{CUST_DETAILS_RUN=null;});
  return CUST_DETAILS_RUN;
}
...
function editCustomer(vat){
  ...
  ceFillDetails(c.vat,c.code);
}
// Η φόρμα ανοίγει αμέσως· τα στοιχεία που λείπουν έρχονται από τη βάση και μετά
// επιβεβαιώνονται στην ΑΑΔΕ. Ό,τι έχει ήδη γράψει ο χρήστης ΔΕΝ πειράζεται.
async function ceFillDetails(vat,code){
  const fill=det=>{if(!det||CE_VAT!==vat)return;
    const map={ceZip:'zip',ceEmail:'email',cePhone1:'phone1',cePhone2:'phone2',ceDoy:'doy',ceJob:'job_description',ceAddress:'address',ceCity:'city'};
    Object.entries(map).forEach(([id,k])=>{const el=$('#'+id);if(el&&!el.value.trim()&&det[k])el.value=det[k];});};
  const q={customer_details:1,buyer_vat:vat||'',customer_code:code||''};
  let stale=true;
  try{const d=await api(q);if(d&&d.cached){fill(d.details);stale=!!d.stale;}}catch(e){}
  try{const d=await api(Object.assign({verify:1},q));if(d&&d.success)fill(d.details);}catch(e){if(stale){}}
}
...
// Το κλειδί σε παραμέτρους: ΑΦΜ ή κωδικός πελάτη — ποτέ και τα δύο.
function cardParam(key){key=(key||'').trim();
  return key.charAt(0)==='#'?{customer_code:key.slice(1)}:{buyer_vat:key};}
...
// Το κρυφό πεδίο κρατά ΚΛΕΙΔΙ, όχι ΑΦΜ: ο ιδιώτης δεν έχει ΑΦΜ και
// αναγνωρίζεται από τον κωδικό πελάτη («#34»). Το ίδιο κλειδί κρατά και τις
// τοπικές πληρωμές του, οπότε η καρτέλα του βγάζει κανονικό υπόλοιπο.
function cardKey(vat,code){vat=(vat||'').trim();code=(code||'').toString().trim();
  return vat||(code?('#'+code):'');}
function openCard(vat,name,code){const k=cardKey(vat,code);showView('card');$('#cardVat').value=k;$('#cardCust').value=name||k;defaultRange();loadCard();}
// Customer selector on the Καρτέλα tab (search by name/ΑΦΜ)
async function cardAc(inp){const panel=$('#cardAcPanel');
  await ensureCustomers();
  const term=grFold(inp.value).trim();
  let rows=ALL_CUSTOMERS.map(custFields);
  ...
  panel.innerHTML=rows.map(c=>`<div class="ac-row" onmousedown="pickCardCust('${q1(c.vat)}','${q1(c.name)}','${q1(c.code)}')"><b>${esc(c.name||c.vat)}</b> <small>${c.vat?('ΑΦΜ '+esc(c.vat)):('ιδιώτης · κωδ. '+esc(c.code||'—'))}${c.city?' · '+esc(c.city):''}</small></div>`).join('');
  panel.classList.add('open');}
function pickCardCust(vat,name,code){const k=cardKey(vat,code);$('#cardVat').value=k;$('#cardCust').value=name||k;$('#cardAcPanel').classList.remove('open');loadCard();}
...
//: Ποια τράπεζα και ποιος λογαριασμός εισέπραξε — ό,τι βρήκε το τελευταίο αρχείο.
let BI_BANK={bank:'',account:'',iban:''};
// Η ίδια λίστα τραπεζών με τις Ρυθμίσεις, ώστε η επιλογή να μπορεί ΠΑΝΤΑ να
// δείξει αυτό που εντοπίστηκε. Πριν είχε τέσσερις επιλογές και η αναγνώριση
// «Πειραιώς» δεν είχε πού να γραφτεί.
function biBankOptions(){
  const sel=$('#biBank');if(!sel)return;
  const banks=(BANK_CFG.banks||[]);
  if(!banks.length||sel.dataset.filled==='1')return;
  const keep=sel.value;
  sel.innerHTML='<option value="">Αυτόματα</option>'+
    banks.map(b=>`<option value="${esc(b.name)}">${esc(b.name)}</option>`).join('');
  sel.value=keep;sel.dataset.filled='1';
}
// Ο λογαριασμός σε ανθρώπινη μορφή: IBAN σε τετράδες, αλλιώς ό,τι βρέθηκε.
function biAccountLabel(a){a=(a||'').trim();return /^GR/i.test(a)?ibanPretty(a):a;}
...
async function biPreview(){const f=$('#biFile').files[0];if(!f){toast('Επίλεξε αρχείο','err');return;}
  ...
  try{const b64=await new Promise((res,rej)=>{const rd=new FileReader();
    ...
    // Η τράπεζα που ΕΝΤΟΠΙΣΤΗΚΕ μπαίνει στην επιλογή, ώστε να είναι ορατή και
    // διορθώσιμη — και κρατιέται για να γραφτεί σε κάθε πληρωμή του αρχείου.
    biBankOptions();
    BI_BANK={bank:d.bank_label||$('#biBank').value||'',account:d.account||'',iban:d.iban||''};
    if(d.bank_label&&!$('#biBank').value)$('#biBank').value=d.bank_label;
    const box=$('#biBankFound');
    if(BI_BANK.bank||BI_BANK.account){
      const how=d.bank_source==='settings'?'από τους λογαριασμούς σου'
               :d.bank_source==='iban'?'από το IBAN του αρχείου'
               :d.bank_source==='text'?'από το κείμενο του αρχείου':'';
      box.innerHTML='🏦 Αναγνωρίστηκε: <b>'+esc(BI_BANK.bank||'άγνωστη τράπεζα')+'</b>'
        +(BI_BANK.account?' · λογαριασμός <b>'+esc(biAccountLabel(BI_BANK.account))+'</b>':'')
        +(how?' <span class="muted">('+how+')</span>':'')
        +'<br><span class="muted">Γράφεται σε κάθε πληρωμή που θα καταχωρηθεί. Άλλαξε την επιλογή αριστερά αν δεν είναι σωστή.</span>';
      box.style.display='';
    }else{
      box.innerHTML='🏦 Δεν αναγνωρίστηκε τράπεζα μέσα στο αρχείο — διάλεξέ την αριστερά για να καταγραφεί στις πληρωμές.';
      box.style.display='';
    }
    $('#biInfo').innerHTML=`Βρέθηκαν <b>${BI_TX.length}</b> κινήσεις · <b>${cr}</b> εισπράξεις · αυτόματη αντιστοίχιση: <b>${matched}</b>`;
...
function biRender(){const onlyCr=$('#biOnlyCredit').checked;
  ...
  biSelInfo();}
async function biAc(inp,i){const panel=$('#biAc'+i);
  await ensureCustomers();
  const term=grFold(inp.value).trim();
  let rows=ALL_CUSTOMERS.map(custFields);
  if(term)rows=rows.filter(c=>grFold([c.vat,c.name,c.code,c.city].join(' ')).includes(term));
...
async function biImport(){const sel=BI_TX.filter(t=>t.include);
  ...
  // Η επιλογή του χρήστη υπερισχύει της αναγνώρισης: αν διόρθωσε την τράπεζα,
  // αυτή γράφεται.
  const bank=$('#biBank').value||BI_BANK.bank||'';
  const items=sel.map(t=>({customer_vat:t.cust?t.cust.vat:'',customer_name:t.cust?t.cust.name:'',customer_code:t.cust?t.cust.code:'',
    ...
    bank,bank_account:BI_BANK.account||'',
...
async function loadCard(){const vat=$('#cardVat').value.trim();if(!vat){toast('Επίλεξε πελάτη','err');return;}defaultRange();
  ...
  try{const d=await api(Object.assign({ledger:1,issue_date_from:di('cardFrom'),issue_date_to:di('cardTo')},cardParam(vat)));if(!d.success)throw new Error(d.error||'σφάλμα');CARD=d;
...
function pmReset(){['pmAmount','pmNotes'].forEach(i=>$('#'+i).value='');$('#pmResult').textContent='';
  ...
  dset('pmDate',new Date().toISOString().slice(0,10));pmBankOptions();pmCardBtn();}
// Οι λογαριασμοί που έχει καταχωρήσει η επιχείρηση (Ρυθμίσεις → 🏦).
function pmBankOptions(keep){
  const sel=$('#pmBankAcc'),wrap=$('#pmBankWrap');if(!sel||!wrap)return;
  const accs=(BANK_CFG.accounts||[]).filter(a=>a.iban);
  wrap.style.display=accs.length?'':'none';
  sel.innerHTML='<option value="">—</option>'+accs.map(a=>
    `<option value="${esc(a.iban)}" data-bank="${esc(a.bank||'')}">${esc((a.bank||'Λογαριασμός')+' · '+ibanPretty(a.iban))}</option>`).join('');
  if(keep)sel.value=ibanNorm(keep);
}
...
function openManualPayment(){
  PM_MODE='manual';
  ensureCustomers();loadBankSettings();
...
async function pmAc(inp){const panel=$('#pmAcPanel');
  await ensureCustomers();
  const term=grFold(inp.value).trim();
  let rows=ALL_CUSTOMERS.map(custFields);
  if(term)rows=rows.filter(c=>grFold([c.vat,c.name,c.code,c.city].join(' ')).includes(term));
...
async function pmShowCard(){
  ...
  try{const d=await api(Object.assign({ledger:1,issue_date_from:y+'-01-01',issue_date_to:y+'-12-31'},cardParam(vat)));
    if(!d.success)throw new Error(d.error||'σφάλμα');
...
async function savePayment(ev){ev.preventDefault();
  ...
  const accSel=$('#pmBankAcc'),accOpt=accSel&&accSel.selectedOptions[0];
  const p={buyer_vat:vat,customer_name:name,pay_amount:$('#pmAmount').value,
           ...
           pay_bank_account:accSel?accSel.value:'',
           pay_bank:(accOpt&&accOpt.dataset.bank)||''};
...
async function loadCategories(pick){
  try{
    const d=await api({list_product_categories:1,force:CACHE_DIRTY.has('prodcats')?1:''});CACHE_DIRTY.delete('prodcats');
...
function openProductModal(prefillCode,onSaved,forceType){PROD_EDIT=null;PROD_ONSAVED=onSaved||null;$('#prodModalTitle').textContent='Νέο είδος';['pdCode','pdDesc','pdTaric'].forEach(i=>$('#'+i).value='');$('#pdPrice').value='0';$('#pdCode').readOnly=false;if(typeof prefillCode==='string')$('#pdCode').value=prefillCode;
  // Νέο είδος = επόμενος ελεύθερος κωδικός, επιλεγμένος ώστε όποιος θέλει δικό
  // του να τον γράψει από πάνω με την πρώτη πληκτρολόγηση.
  if(!$('#pdCode').value){$('#pdCode').value=nextProductCode();setTimeout(()=>{try{$('#pdCode').select();}catch(e){}},80);}$('#pdType').value=forceType==='good'?'1':'2';pdTypeChange();loadCategories();$('#prodModal').showModal();}
/**
...
async function saveCatCls(){const name=$('#ccName').value.trim();if(!name){$('#ccErr').textContent='Λείπει η ονομασία.';return;}
  ...
  try{const d=await api({save_category_cls:1,category_id:CATCLS_EDIT||0,category_name:name,cls:JSON.stringify(cls)});
    if(!d.success)throw new Error(d.error||'σφάλμα');$('#catClsModal').close();toast('Χαρακτηρισμοί αποθηκεύτηκαν','ok');cacheTouch('categories');cacheTouch('prodcats');loadCatCls();loadCategories();
...
function blkAddRow(vat='',name='',code='',qty=1,price=''){
  ...
  tr.innerHTML=`
    ...
    <td><input class="blk-price" type="text" inputmode="decimal" value="${esc(price)}" onblur="elFmtField(this,2)" placeholder="0,00" style="width:110px;text-align:right"></td>
...
function blkPickProd(el,code){const td=el.closest('td');const inp=td.querySelector('.blk-code');inp.value=prodText(code);inp.dataset.code=code;td.querySelector('.ac-panel').classList.remove('open');
  const tr=el.closest('tr');const priceInp=tr.querySelector('.blk-price');const p=PRODMAP[code];if(p&&p.price&&!elNum(priceInp.value))priceInp.value=elFmt(p.price,2);blkCountUpd();}
function blkNewProd(el){const td=el.closest('td');const inp=td.querySelector('.blk-code');td.querySelector('.ac-panel').classList.remove('open');
  ...
  openProductModal(/^[\w.-]{1,20}$/.test(typed)&&!PRODMAP[typed]?typed:'',code=>{inp.value=prodText(code);inp.dataset.code=code;
    const tr=td.closest('tr');const priceInp=tr.querySelector('.blk-price');const p=PRODMAP[code];if(p&&p.price&&!elNum(priceInp.value))priceInp.value=elFmt(p.price,2);blkCountUpd();});}
...
function blkImportCsv(){const raw=($('#blkCsv').value||'').split(/\r?\n/);let added=0;
  raw.forEach(ln=>{const s=ln.trim();if(!s||s.startsWith('#'))return;const p=s.split(/[;,\t]/).map(x=>x.trim());if(p.length<4)return;
    ...
    blkAddRow(afm,c?c.name:'',code,parseFloat((qty||'').replace(',','.'))||1,price?elFmt(parseFloat(price.replace(',','.'))||0,2):'');added++;});
...
function lineRowHtml(code,qty,price,disc){return `<tr>
  ...
  <td class="num"><input class="ln-price" type="text" inputmode="decimal" value="${esc(price)}" oninput="lineFromInputs(this.closest('tr'))" onblur="elFmtField(this,2);lineFromInputs(this.closest('tr'))" placeholder="0,00" style="width:100px;text-align:right"></td>
...
function pickProd(el,code){const row=el.closest('tr');const inp=row.querySelector('.ln-code');const p=PRODMAP[code]||{};
  ...
  if(p.price&&!elNum(row.querySelector('.ln-price').value))row.querySelector('.ln-price').value=elFmt(elNum(p.price),2);
...
function newProdForLine(el){const row=el.closest('tr');const inp=row.querySelector('.ln-code');row.querySelector('.ln-ac').classList.remove('open');
  const typed=(inp.value||'').trim();
  // Ό,τι πληκτρολόγησε ο χρήστης: αν μοιάζει με κωδικό γίνεται κωδικός, αλλιώς
  // περιγραφή — και ο κωδικός παίρνει τον επόμενο διαθέσιμο αριθμό.
  const asCode=/^[\w.-]{1,20}$/.test(typed)&&/\d/.test(typed)&&!PRODMAP[typed];
  // ⚠️ Η ΓΡΑΜΜΗ κρατιέται τώρα, όχι το στοιχείο που πατήθηκε: η λίστα
  // προτάσεων ξαναζωγραφίζεται, το κουμπί «➕ Νέο είδος» φεύγει από τη σελίδα,
  // και η επιλογή του νέου είδους έσκαγε σιωπηλά μετά την αποθήκευση.
  openProductModal(asCode?typed:'',code=>{pickProd(row.querySelector('.ln-code'),code);});
  if(!asCode&&typed)setTimeout(()=>{if(!$('#pdDesc').value)$('#pdDesc').value=typed;},0);}
// Ο επόμενος ελεύθερος αριθμητικός κωδικός είδους (μέγιστος + 1).
function nextProductCode(){
  const codes=Object.keys(PRODMAP).concat((PRODUCTS||[]).map(p=>String(p.product_code||p.code||'')));
  let max=0;codes.forEach(c=>{if(/^\d{1,9}$/.test(c))max=Math.max(max,parseInt(c,10));});
  let n=max+1;const taken=new Set(codes);while(taken.has(String(n)))n++;
  return String(n);
}
...
function lineFromInputs(row){const u=price2(row.querySelector('.ln-price').value);const q=parseFloat(row.querySelector('.ln-qty').value)||0;const d=Math.min(parseFloat(row.querySelector('.ln-disc').value)||0,100);const r=rowRate(row);
...
function lineFromGross(row){const G=elNum(row.querySelector('.ln-gross').value);const q=parseFloat(row.querySelector('.ln-qty').value)||0;const r=rowRate(row);
  ...
  if(q>0&&u>0){const baseNet=u*q;
    ...
    else{discInp.value='';priceInp.value=elFmt(unitForGross(G,q,r),2);}
  }else if(q>0){priceInp.value=elFmt(unitForGross(G,q,r),2);discInp.value='';}
  sumTotals();}
// Τιμή μονάδας ΜΕ 2 ΔΕΚΑΔΙΚΑ που φτάνει όσο πιο κοντά γίνεται στο σύνολο που
// πληκτρολόγησε ο χρήστης. Η ΑΑΔΕ δεν δέχεται 2419,3548: κόβει την οριστική
// έκδοση. Κάποια σύνολα (π.χ. 3.000 με 24%) δεν βγαίνουν ακριβώς με λεπτά —
// τότε διαλέγουμε τη μικρότερη απόκλιση (±0,01 €).
function unitForGross(G,q,r){
  if(!(q>0))return 0;
  const exact=G/(1+r)/q,lo=Math.floor(exact*100)/100,hi=Math.ceil(exact*100)/100;
  const gross=u=>{const net=Math.round(u*q*100)/100;return net+Math.round(net*r*100)/100;};
  return Math.abs(gross(hi)-G)<Math.abs(gross(lo)-G)-1e-9?hi:lo;
}
function price2(v){return Math.round(elNum(v)*100)/100;}
function collectLines(){const out=[];document.querySelectorAll('#iLines tbody tr').forEach(r=>{
  const code=(r.querySelector('.ln-code').dataset.code||'').trim();const qty=parseFloat(r.querySelector('.ln-qty').value)||0;const price=price2(r.querySelector('.ln-price').value);const disc=parseFloat(r.querySelector('.ln-disc').value)||0;
...
function sumTotals(){let net=0,vat=0;document.querySelectorAll('#iLines tbody tr').forEach(r=>{
  const u=price2(r.querySelector('.ln-price').value);const q=parseFloat(r.querySelector('.ln-qty').value)||0;const d=Math.min(parseFloat(r.querySelector('.ln-disc').value)||0,100);const rate=rowRate(r);
  const ln=Math.round(u*q*(1-d/100)*100)/100;net+=ln;vat+=Math.round(ln*rate*100)/100;
...
// Ό,τι δείχνει η γραμμή, σε ένα κείμενο. Τα ποσά μπαίνουν ΚΑΙ όπως γράφονται
// («10.000,00») ΚΑΙ σκέτα («10000»), ώστε να βρίσκονται είτε τα πληκτρολογήσεις
// με τελείες είτε χωρίς. Το ίδιο η ημερομηνία: «24/08/2026» και «2026-08-24».
function docHay(i){
  const num=v=>{const n=elNum(v);return n?(' '+n+' '+n.toFixed(2)):'';};
  return grFold([
    i.mark,i.series,i.aa,i.type,
    i.counterpart_vat,i.buyer_vat,i.counterpart,i.counterpart_name,docWho(i),
    i.issue_date,dtParse(i.issue_date||''),
    i.net_value,i.vat_value,i.total,
  ].join(' ')+num(i.net_value)+num(i.vat_value)+num(i.total));
}
function docRows(){
  const term=grFold($('#docSearch').value||'').trim();
  if(!term)return ALL_DOCS;
  return ALL_DOCS.filter(i=>docHay(i).includes(term));
}
function renderDocs(){
  const rows=docRows();
...
let MM_CTX=null;
// «Καλημέρα» ως τις 12:00, «Καλησπέρα» μετά. Ο server τον ξαναδιορθώνει τη
// στιγμή που το μήνυμα πραγματικά φεύγει.
function mailGreeting(d){return (d||new Date()).getHours()<12?'Καλημέρα':'Καλησπέρα';}
// Η επωνυμία της εταιρείας για την υπογραφή — το «(802576637)» του επιλογέα φεύγει.
function mailCompany(){
  const t=(($('#account option:checked')||{}).textContent||'').trim();
  return t.replace(/\s*[\(\[]?\s*(ΑΦΜ[:\s]*)?\d{9}\s*[\)\]]?\s*$/,'').trim();
}
function mailHello(name){const n=String(name||'').trim();return mailGreeting()+(n&&!/^#?\d{9}$/.test(n)?' '+n:' σας')+',';}
function mailDefaults(kind,info){
  const company=mailCompany();
  const who=(info.name||'').trim();
  const sign=company?('\n\nΜε εκτίμηση,\n'+company):'';
  const hello=mailHello(who);
  if(kind==='doc'){
...
function emailDocument(mark){
  ...
  openMailDialog('doc',{mark,series:row.series,aa:row.aa,date:row.issue_date,
    total:row.total,name:(()=>{const w=docWho(row);return /^\d{9}$|^—$/.test(w)?'':w;})(),
...
async function loadBankSettings(force){
  ...
  try{
    ...
    if($('#bkTable'))bkRender();
    // Οι λογαριασμοί τροφοδοτούν ΚΑΙ την εισαγωγή πληρωμών: μόλις έρθουν,
    // γεμίζουν η λίστα τραπεζών και ο επιλογέας λογαριασμού είσπραξης.
    biBankOptions();
    if($('#pmBankAcc'))pmBankOptions($('#pmBankAcc').value);
  }catch(e){}
}
...
async function lbSend(){
  ...
  for(let i=0;i<picks.length;i++){
    ...
    try{
      ...
      const body=cardMailBody({name:r.name||'',period,balance:r.balance});
...
function ledgerFileName(name,vat){vat=(vat||'').charAt(0)==='#'?('ΚΩΔ '+vat.slice(1)):vat;
...
const CB_NAV=[
  // Η μαζική ΠΡΙΝ την έκδοση: το «μαζική έκδοση» περιέχει «έκδοση».
  ['bulk',         ['μαζικη εκδοση','μαζικ','παρτιδ','bulk','mass issue','issue many']],
  ...
  ['card',         ['καρτελ','υπολοιπ','κινησεισ πελατ','customer card','client card','ledger']],
  ['customers',    ['πελατ','πελατολογ','customers','customer','clients','client list']],
  ...
  ['drafts',       ['προχειρ','προσχεδ','drafts','saved drafts']],
  ['cancel',       ['ακυρωσ','πιστωτικ','credit note','cancel an invoice','cancellation']],
  ['schedule',     ['προγραμματισμ','χρονοπρογραμμ','αυτοματη εκδοσ',
                    'schedule','scheduler','scheduled invoice']],
  ['stats',        ['στατιστικ','τζιρο','γραφημ','πιτα','statistics','stats','charts']],
  ...
  ['admin',        ['διαχειρισ','χρηστ','ρολο','προσκλησ','λογιστ','εταιρει','επιχειρησ',
                    'administration','users and roles','manage users','companies']],
];
...
const CB_VIEW_NAMES={issue:'Έκδοση',bulk:'Μαζική έκδοση',customers:'Πελάτες',card:'Καρτέλα',
  ...
  settings:'Ρυθμίσεις',admin:'Διαχείριση',notifications:'Ειδοποιήσεις'};
// (μοτίβο, ενότητα, τι λέει) για λειτουργίες που είναι κουμπιά ΜΕΣΑ στη σελίδα —
// το ✕ της ειδοποίησης, οι «Στήλες», η «Επαναφορά». Ο βοηθός δεν τις εκτελεί:
// ανοίγει τη σωστή οθόνη και λέει τι να πατήσει ο χρήστης. Είναι πιο χρήσιμο
// και από «δεν κατάλαβα», και από μια ενέργεια που δεν υπάρχει.
const CB_TIPS=[
  [/(?:σβην|σβησ|διαγραφ|διαγραψ|καθαρι)\S*\s+\S*\s?ειδοποιησ|delete a notification|clear the notification/,
   'notifications','Κάθε ειδοποίηση έχει ένα ✕ πάνω δεξιά που τη σβήνει οριστικά. Το κλικ στη γραμμή σημαίνει μόνο «διαβασμένη» — είναι άλλο πράγμα.'],
  [/ανανεωσ\S*\s+(?:τα\s+)?παραστατ|τσεκαρε τον φακελο|ελεγξε τον φακελο|λεει αναμονη|ενω κατεβηκε|refresh the document|check the folder|says? pending/,
   'documents','Πάτα «Ανανέωση»: ελέγχει τον φάκελο του πελάτη και ό,τι βρει εκεί το σημειώνει ως «Ελήφθη».'],
  [/επαναφορα|να επαναφερω|παλιο αντιγραφο|restore/,
   'settings','Η «Επαναφορά από αντίγραφο» είναι στις Ρυθμίσεις. Ζητά να γράψεις τη λέξη ΕΠΑΝΑΦΟΡΑ πριν αγγίξει οτιδήποτε.'],
  [/δοκιμασε τα κλειδι|ελεγξε αν δουλευει το api|τεστ κωδικ|test the api|check the credential/,
   'customers','Στην καρτέλα του πελάτη, δίπλα στο κλειδί, υπάρχει κουμπί «Δοκιμή» που ρωτά την ΑΑΔΕ επιτόπου.'],
  [/διαλεξε στηλ|κρυψε (?:μια )?στηλ|θελω αλλεσ στηλ|choose column|hide a column/,
   '','Πάνω από κάθε πίνακα υπάρχει το κουμπί «⚙ Στήλες»: διαλέγεις τι φαίνεται.'],
  [/αλλαξε θεμα|φωτεινο θεμα|σκουρο θεμα|change the theme|dark mode|light mode/,
   '','Ο διακόπτης «Φωτεινό θέμα» είναι κάτω αριστερά, στο πλαϊνό μενού.'],
  [/τι σημαινει ο τυπο|τι ειναι το \d\.\d|τι τυποσ παραστατικου|μονο τα \d\.\d/,
   'documents','Ο κωδικός τύπου είναι του myDATA: το «2.1» είναι Τιμολόγιο Παροχής Υπηρεσιών.'],
];
...
async function cbHandle(t){const s=cbNorm(t);
  ...
  if(/μιλα μου|ξαναμιλα|ενεργοποιησε τη φωνη|ανοιξε τη φωνη|\bunmute\b|talk to me|turn the voice on/.test(s)){
    CB_MUTED=false;cbBot('Εντάξει, ξαναμιλάω.');return;}
  // Λειτουργίες που είναι ΚΟΥΜΠΙΑ μέσα στη σελίδα, όχι ενέργειες του
  // κελύφους: ο βοηθός πάει στη σωστή οθόνη και λέει τι να πατήσεις.
  for(const[re_,view,say]of CB_TIPS){
    if(re_.test(s)){if(view)showView(view);cbBot(say);return;}
  }
  if(/εγχειριδι|manual|οδηγιεσ χρησ|user guide|documentation/.test(s)){
    cbBot('Ετοιμάζω το εγχειρίδιο χρήσης σε PDF.');downloadManual();return;}
  if(/αντιγραφο ασφαλει|backup|back up|κρατα \S*\s?αντιγραφο|save my data/.test(s)){
  ...
  if(/ανανεωσ|ξαναφορτωσ|refresh|reload|φρεσκαρε/.test(s)){
    const nav=document.querySelector('.nav-item.active');
    if(nav)showView(nav.dataset.view);
    cbBot('Ανανέωσα τη σελίδα.');return;}
  if(/αποσυνδεσ|logout|log out|sign out|sign me out|βγαλε με εξω/.test(s)){cbBot('Σε αποσυνδέω.');logout();return;}
  // Πλήθη: τα έχουμε ήδη στη μνήμη — καμία ερώτηση στο backend, και ΠΡΙΝ τα
  // στατιστικά, που μετρούν παραστατικά και όχι πελάτες.
  if(/ποσ\S*\s+(?:\w+\s+){0,2}πελατ|how many (?:\w+\s+){0,2}(?:customers|clients)|customer count|client count/.test(s)){
    await ensureCustomers();
    cbBot('Έχεις '+ALL_CUSTOMERS.length+' πελάτες στο πελατολόγιο.');return;}
  if(/ποσ\S*\s+(?:ειδ|προιοντ)|how many items|how many products|product count|item count/.test(s)){
  ...
  const isNav=/πηγαινε|ανοιξε|δειξε|εμφανισε|παμε|βγαλε μου|φερε μου|go to|open|show|list|take me/.test(s);
  if(isNav){
    const view=cbMatchView(s);
    if(view){showView(view);cbBot('Άνοιξα: '+(CB_VIEW_NAMES[view]||view)+'.');return;}
  }
  let isIssue=!isNav&&/εκδοσ|τιμολογ|αποδειξ|παραστατ|issue|invoice/.test(s)&&!/λιστα|αναζητησ|ολα τα/.test(s);
  // Λέξη τιμολόγησης ΧΩΡΙΣ τίποτα να εκδοθεί (ούτε ΑΦΜ, ούτε ποσό) είναι
  // όνομα οθόνης, όχι εντολή: «τα παραστατικά», «μαζική έκδοση»,
  // «cancel an invoice». Η Έκδοση κρατά μόνο ό,τι δείχνει σε αυτήν.
  if(isIssue&&!/\d/.test(s)){
    const view=cbMatchView(s);
    if(view&&view!=='issue'){showView(view);
      cbBot('Άνοιξα: '+(CB_VIEW_NAMES[view]||view)+'.');return;}
  }
  // Help
...
async function cbDoIssue(o){cbBot('Ετοιμάζω το πρόχειρο…');
  ...
  if(o.price){row.querySelector('.ln-price').value=elFmt(elNum(o.price),2);lineFromInputs(row);}
...
const MANUAL=[
  ...
  ['👤 <b>Πελάτες</b> — ο Τ.Κ., το email, τα τηλέφωνα και η ΔΟΥ διαβάζονται από τη σελίδα κάθε πελάτη στην ΑΑΔΕ, μία φορά, και μένουν αποθηκευμένα· ξαναελέγχονται στο παρασκήνιο. Χωρίς Τ.Κ. η ΑΑΔΕ απορρίπτει την οριστική έκδοση, οπότε το παραστατικό βγαίνει τότε χωρίς διεύθυνση λήπτη και σε ειδοποιεί.','li'],
  ['💶 <b>Τιμές</b> — πάντα με 2 δεκαδικά (η ΑΑΔΕ δεν δέχεται περισσότερα). Αν γράψεις σύνολο με ΦΠΑ, η τιμή μονάδας βγαίνει όσο πιο κοντά γίνεται· κάποια σύνολα (π.χ. 3.000 € με ΦΠΑ 24%) δεν βγαίνουν ακριβώς και διαφέρουν κατά 1 λεπτό.','li'],
  ['📊 <b>Στατιστικά</b> — κάθε ποσό λέει τι είναι: <b>καθαρή αξία</b> (χωρίς ΦΠΑ), ΦΠΑ, και <b>σύνολο με ΦΠΑ</b>. Τα πιστωτικά αφαιρούνται.','li'],
  ['📦 <b>Νέο είδος</b> — παίρνει αυτόματα τον επόμενο ελεύθερο κωδικό (τον αλλάζεις αν θέλεις δικό σου). Όταν το φτιάχνεις μέσα από την Έκδοση, μπαίνει μόνο του στη γραμμή μόλις αποθηκευτεί.','li'],
  ['✉️ <b>Email</b> — το μήνυμα ξεκινά με «Καλημέρα» ή «Καλησπέρα» ανάλογα με την ώρα που φεύγει, και την επωνυμία του πελάτη· η υπογραφή γράφει την επωνυμία της εταιρείας χωρίς ΑΦΜ.','li'],
