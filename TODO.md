...
- [x] **Column filters + row selection** (checkboxes, select-all) + restyle ALL tables to the *timologio-downloader* look.
  - [x] Table restyle to the grid look: sticky headers, zebra rows (`nth-child(even)`), stronger header underline, hover — applied globally via the base `table`/`thead th` CSS.
  - [x] Reusable **per-column filter row**: `attachColumnFilters(tableId)` injects a second header row of filter inputs; `applyColumnFilters()` hides non-matching rows (non-invasive, works with any custom renderer). Wired into Πελάτες, Είδη, Πρόχειρα, Ακύρωση tables (re-applied after each render). Columns marked `nofilter`/`grid-check` are skipped.
  - [x] **Row checkboxes + select-all + bulk delete** on Πρόχειρα (`df-chk`/`dfAll`/`dfToggleAll`/`delSelectedDrafts`) — solves the earlier one-by-one draft deletion pain.
  - [ ] Optional: extend checkboxes/bulk actions to other lists if needed.
...
- [x] Perf: new-category-with-suggested-classification was slow — it fired ~24 `cls_options` requests **sequentially**. Now (a) client fetches all invoice types' options **in parallel** (`Promise.all`), and (b) the bridge **caches** each `cls_options` result (`clsopt:{type}:{self}`) and serves it **without an AADE login** on repeats. Measured: first call ~1.38s → cached ~0.11s (~12× faster).
...
- [x] **Page tour + PDF manual**: header buttons «🧭 Ξενάγηση» (spotlight overlay stepping through Έκδοση/Μαζική/Πελάτες/Καρτέλα/Πληρωμές/Σειρές/Πρόχειρα/Ctrl-K/θέμα, `startTour`/`tourShow`) and «📄 Εγχειρίδιο» (jsPDF + DejaVu Greek font, `downloadManual`). First-time visitors are offered the tour once (localStorage `etim_tour_done`).
