...
// --- 6β. Μορφή του κλειδιού σύνδεσης ----------------------------------------
// Το κλειδί κουβαλά τη διεύθυνση του server μέσα του· αν σπάσει αυτή η μορφή,
// καμία εγκατάσταση γραφείου δεν ξαναδένεται — και το λάθος φαίνεται μόνο στον
// πελάτη. Ο έλεγχος ζει εδώ γιατί εδώ τρέχει ο κώδικας (PHP), όχι στα python
// tests της εφαρμογής υπολογιστή.
section('6β. Κλειδί σύνδεσης με server');
require_once $ROOT . '/serverlink.php';
$host = 'timologio.example.gr';
$tok  = 'etim1_' . rtrim(strtr(base64_encode($host), '+/', '-_'), '=') . '_abc123';
[$base, $secret] = link_decode_key($tok);
check('η διεύθυνση βγαίνει από το κλειδί', $base === 'https://' . $host, $base);
check('το μυστικό βγαίνει από το κλειδί', $secret === 'abc123');
[$lb, ] = link_decode_key('etim1_' . rtrim(strtr(base64_encode('127.0.0.1:8391'), '+/', '-_'), '=') . '_x');
check('τοπική διεύθυνση → http (όχι https)', $lb === 'http://127.0.0.1:8391', $lb);
$bad = 0;
foreach (['', 'σκουπίδια', 'etim2_x_y', 'etim1_x', 'etim1_' . base64_encode('x') . '_'] as $b) {
    [$u, $k] = link_decode_key($b);
    if ($u === '' && $k === '') $bad++;
}
check('τα κακοσχηματισμένα κλειδιά απορρίπτονται', $bad === 5, $bad . '/5');
