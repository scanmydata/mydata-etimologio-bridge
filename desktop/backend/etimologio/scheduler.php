...
// --- Το ημερήσιο αντίγραφο του server ---------------------------------------
// Ζει στον ίδιο παλμό με τις εκδόσεις: ο tick τρέχει ανά λεπτό ούτως ή άλλως,
// και ένα δεύτερο cron θα ήταν ένα ακόμη πράγμα που μπορεί να μη στηθεί.
//
// ⚠️ ΕΔΩ, ΠΡΙΝ από κάθε `exit()`. Ήταν γραμμένο στο ΤΕΛΟΣ του αρχείου, κάτω από
// τις βοηθητικές συναρτήσεις — και οι δηλώσεις συναρτήσεων ανεβαίνουν, οι
// εντολές όχι. Ο κώδικας ήταν απροσπέλαστος: το «δεν υπάρχουν εργασίες» βγαίνει
// με `exit(0)` δύο γραμμές παρακάτω, δηλαδή σχεδόν πάντα. Το αυτόματο αντίγραφο
// δεν είχε τρέξει ποτέ, ούτε μία φορά, και κανένα μήνυμα δεν το έλεγε.
