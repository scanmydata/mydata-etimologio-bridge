        ...
        case 'link_sync': case 'backup_status': case 'backup_run': {
            ...
            switch ($authAction) {
                ...
                case 'link_connect': {
                    ...
                    if ($key === '') jsonError('Επικόλλησε το κλειδί πρόσβασης');
                    // Το κλειδί κουβαλά τη διεύθυνση (etim1_<base64 host>_<μυστικό>),
                    // γι' αυτό δεν ζητάμε URL: ο λογιστής δεν χρειάζεται να ξέρει
                    // πού ζει ο server, μόνο να επικολλήσει ό,τι του έδωσαν.
                    [$base, $secret] = link_decode_key($key);
...
if (!empty($_GET['tts'] ?? $_POST['tts'] ?? '')) {
    ...
    // Ο Piper διαβάζει ό,τι του δώσεις: το «802576637» σαν έναν τεράστιο
    // αριθμό, το «12.100,00 €» σαν «δώδεκα κόμμα εκατό», το «ΑΦΜ» σαν «αφμ».
    // Ο κανονικοποιητής τα γράφει ολογράφως — και πεζά, γιατί τα ελληνικά
    // κεφαλαία δεν έχουν τόνο και η μηχανή δεν ξέρει πού πέφτει η έμφαση.
    // Ίδια λογική με το `speakable.py` της εφαρμογής υπολογιστή, ελεγμένη ότι
    // βγάζει το ίδιο κείμενο.
    require_once __DIR__ . '/speakable.php';
    $ttsText = to_speech($ttsText);
    if ($ttsText === '') jsonError('Δεν έμεινε κείμενο για εκφώνηση');
